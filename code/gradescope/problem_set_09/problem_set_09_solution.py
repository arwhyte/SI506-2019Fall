...
for i in films_dict['results']:
    char_count = 0
    for j in i['characters']:
        char_count += 1
    title_id_chars.append((i['title'],i['episode_id'],char_count))
...
def call_API(category, id):
    if id is None:
        url = f"https://swapi.co/api/{category}/"
    else:
        url = f"https://swapi.co/api/{category}/{id}/"
...
for parameter in ["name","height","mass","hair_color","skin_color","eye_color","birth_year","gender"]:
    r2_d2[parameter] = search_dict["results"][0][parameter]
...
def call_API_updated(category, option):
    ...
    if option is None:
        ...
        url = f"https://swapi.co/api/{category}/{option}"
...
# BEGIN PROBLEM 6
films = []
for url in search_dict["results"][0]["films"]:
    parts = url.split("/")
    category = parts[-3]
    option = int(parts[-2])
    res_dict = call_API_updated(category, option)
    films.append(res_dict["title"])

r2_d2["films"] = films

with open("r2_d2.json", 'w', encoding='utf8') as file_obj:
    json.dump(r2_d2, file_obj, ensure_ascii = False, indent = 2)
