...
def write_json(filename, data):
    ...
    The built-in open() function optional parameter value encoding='utf-8-sig' also works.

    Parameters:
        filename (str): the name of the file (e.g., swapi_output.py).
...
def get_films(url, params=None):
    """
    Get a list of dictionaries containing information about films from the given url.
    ...
    Returns:
        films (list): a list of dictionaries, each of which describes a film
    """
...
def filter_properties(film):
    """
    Extract specific properties of the given film into a new dictionary.

    Parameters:
        film (dict): the dictionary containing properties of a film.
    ...
    # Extracts properties listed in <FILM_PROPERTIES> of <film> into <record>
...
def main():
    """
    Write a list of records of films into a JSON file
    ...
    for film in films:
        new_record = filter_properties(film)
        new_film_records.append(new_record)
    # END: WRITE CODE ABOVE
